...
class PreparationJobTests(unittest.TestCase):
    ...
    def test_old_analysis_version_is_not_reused(self):
        ...
        with connect_db() as conn:
            ...
            conn.execute("insert into analyses (analysis_id, video_id, learner_level, transcript_id, cache_key, status, stage, created_at, updated_at) values (?, ?, ?, ?, ?, ?, ?, ?, ?)", ("old-analysis", "stale-analysis-demo", "beginner", "old-transcript", "stale-analysis-demo:old-hash:beginner:agent-mvp-gemini-v2", "completed", "ready", timestamp, timestamp))
