...
def run_whole_video_asr(job_id, video_id):
    ...
    with ASR_LOCK:
        ...
        for chunk in chunks:
            ...
            try:
                segments = provider.transcribe(normalized_audio, chunk, str(job_media_dir), job_id)
        ...
        transcript = store_transcript(video_id, f"{video_id}.whole-video.whisper.vtt", source="whisper_asr", segments=merged, metadata={"provenance": "whisper_asr", **provider.metadata()})
